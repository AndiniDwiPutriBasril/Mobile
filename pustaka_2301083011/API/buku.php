...
switch($method){
    ...
    case 'POST' :
        ...
        if (!empty($data['kode_buku']) && !empty($data['judul']) && !empty($data['pengarang']) && !empty($data['penerbit']) && !empty($data['tahun_terbit'])) {
            $stmt = $pdo->prepare("INSERT INTO buku (kode_buku, judul, pengarang, penerbit, tahun_terbit) VALUES (?, ?, ?, ?, ?)");
            $stmt->execute([$data['kode_buku'], $data['judul'], $data['pengarang'], $data['penerbit'], $data['tahun_terbit']]);
    ...
    case 'PUT' :
        ...
        if ($id) {
            ...
            if ($anggota) {
                ...
                $stmt = $pdo->prepare("UPDATE buku SET kode_buku = ?, judul = ?, pengarang = ?, penerbit = ?, tahun_terbit = ? WHERE id = ?");
                $stmt->execute([$kode_buku, $judul, $pengarang, $penerbit, $tahun_terbit, $id]);
                echo json_encode(["message" => "Buku update"]);
            }else {
                http_response_code(404);
                echo json_encode(["message" => "Buku not found"]);
            }
    ...
    case 'DELETE' :
        ...
        if ($id) {
            ...
            if($buku){
                ...
                $stmt->execute([$id]);
                echo json_encode(["message" => "Buku delete"]);
            } else {
                http_response_code(404);
