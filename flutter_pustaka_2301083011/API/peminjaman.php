...
switch($method){
    case 'GET':
        if($id) {
            $stmt = $pdo->prepare("
                ...
                FROM peminjaman
                JOIN anggota ON peminjaman.anggota = anggota.id
                JOIN buku ON peminjaman.buku = buku.id
                WHERE peminjaman.id = ?
            ");
            $stmt->execute([$id]);
    ...
    case 'POST':
        ...
        if (!empty($data['tanggal_pinjam']) && !empty($data['tanggal_kembali']) && 
            !empty($data['anggota']) && !empty($data['buku'])) {
            $stmt = $pdo->prepare("INSERT INTO peminjaman (tanggal_pinjam, tanggal_kembali, anggota, buku) 
                                 VALUES (?, ?, ?, ?)");
            $stmt->execute([
                $data['tanggal_pinjam'], 
            ...
            echo json_encode(["message" => "Peminjaman berhasil ditambahkan", "id" => $pdo->lastInsertId()]);
        } else {
    ...
    case 'PUT':
        if ($id) {
            ...
            if ($peminjaman) {
                ...
                $stmt = $pdo->prepare("UPDATE peminjaman SET tanggal_pinjam = ?, tanggal_kembali = ?, 
                                     anggota = ?, buku = ? WHERE id = ?");
                $stmt->execute([$tanggal_pinjam, $tanggal_kembali, $anggota, $buku, $id]);
                echo json_encode(["message" => "Data peminjaman berhasil diperbarui"]);
            } else {
                http_response_code(404);
                echo json_encode(["message" => "Peminjaman tidak ditemukan"]);
            }
